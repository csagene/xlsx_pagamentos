...
# ---------------------------------------------------------
# CONFIGURA├ç├âO GERAL
# ---------------------------------------------------------
st.set_page_config(page_title="Sistema de Relat├│rios INAS", page_icon="­ƒôè", layout="wide")

TEMPLATES_FILE = "templates.json"

DEFAULT_TEMPLATES = {
    "modelo de tabela globalizadovf_final_xls": {
        "colunas_agrupamento": ["Ano ", "M├¬s", "Prov├¡ncia", "Delega├º├úo", "Distrito", "Fonte", "Programa", "Implementador", "Provedor  servico"],
...
# Inicializar vari├íveis na mem├│ria
...
# ---------------------------------------------------------
# MENU DE NAVEGA├ç├âO LATERAL
# ---------------------------------------------------------
PAGINAS = [
    "­ƒôÑ 1. Carregar & Editar Dados", 
    "­ƒôæ 2. Gerar Relat├│rio", 
    "­ƒôê 3. Dashboard Visual"
]

if "pagina_atual" not in st.session_state:
    st.session_state.pagina_atual = PAGINAS[0]

st.sidebar.title("­ƒº¡ Organiza├º├úo")
st.sidebar.markdown("Navegue entre as p├íginas da aplica├º├úo:")
...
if st.session_state.df_editado is not None:
    st.sidebar.success(f"Ô£à Dados carregados ({len(st.session_state.df_editado)} registos)")
else:
    st.sidebar.warning("ÔÜá´©Å Nenhum dado carregado")
...
def processar_relatorio(df, template):
    ...
    aliases = {
        ...
        "m├¬s": ["meses_pagamento", "mes", "m├¬s", "meses", "month"],
        "prov├¡ncia": ["provincia", "prov├¡ncia", "province"],
        "delega├º├úo": ["delegacao", "delega├º├úo"],
    ...
    col_beneficiario = detetar_coluna(colunas_df, ['id', 'c├│digo', 'codigo', 'beneficiario', 'benefici├írio', 'bi', 'nuit'])
    col_sexo = detetar_coluna(colunas_df, ['sexo', 'genero', 'g├®nero', 'sex'])
    ...
    if col_sexo:
        ...
        for sexo_col in sexo_pivot.columns:
            ...
            if val == 'F':
                ...
                nome_col = f"Sexo_{val}" if val != "SEM_GENERO" else "Sem_G├¬nero"
    ...
    for col in relatorio_final.columns:
        if "ano" in str(col).lower():
            # Converter para string e remover '.0' de floats para n├úo aparecer com v├¡rgulas/decimais na UI
    ...
    for col in col_agrupamento:
        if 'mes' in col.lower() or 'm├¬s' in col.lower():
            col_mes = col
            break
    ...
    if col_mes:
        ...
        # Ordenar pelos agrupamentos base e depois pelo m├¬s, para o cumsum fazer sentido!
...
# =========================================================
# P├üGINA 1: CARREGAR & EDITAR DADOS
# =========================================================
if pagina == PAGINAS[0]:
    st.title("­ƒôÑ 1. Carregar Dados")
    st.markdown("Fa├ºa o upload do ficheiro Excel com os dados em bruto. Pode visualizar a informa├º├úo importada antes de gerar o relat├│rio.")
    ...
    if uploaded_file is not None:
        if st.session_state.get('last_file_name') != uploaded_file.name:
            with st.spinner("ÔÅ│ A analisar o ficheiro"):
                try:
    ...
    if st.session_state.df_editado is not None:
        st.success("Ô£à Dados em bruto importados com sucesso! (Modo de Apenas Leitura)")
        st.dataframe(st.session_state.df_editado, use_container_width=True, height=500, hide_index=True)

# =========================================================
# P├üGINA 2: GERAR RELAT├ôRIO
# =========================================================
elif pagina == PAGINAS[1]:
    st.header("­ƒôæ Relat├│rios de Pagamentos")
    
    if st.session_state.df_editado is None:
        st.warning("ÔÜá´©Å Volte ├á primeira p├ígina e carregue um ficheiro Excel.")
    elif st.session_state.relatorio_final is not None:
        st.markdown("#### ­ƒöì Filtros em Cascata")
        st.markdown("Selecione op├º├Áes abaixo para filtrar os detalhes. Os totais atualizar├úo automaticamente.")
        ...
        if len(st.session_state.col_agrupamento) > 0:
            ...
            for i, col in enumerate(st.session_state.col_agrupamento):
                ...
                with col_filtros[i % 4]:
                    is_mes = "mes" in str(col).lower() or "m├¬s" in str(col).lower()
        ...
        def destacar_totais_isolados(row):
            # A primeira coluna (iloc[0]) agora ├® 'Totais'
        ...
        tab1, tab2 = st.tabs(["­ƒôè Tabela 1 ÔÇö PAGAMENTOS MENSAL", "­ƒôê Tabela 2 ÔÇö PAGAMENTOS CUMULATIVO"])
        ...
        with tab1:
            ...
            st.write("­ƒôî **TOTAIS GERAIS DOS DADOS ACIMA:**")
            ...
            st.download_button(
                label="­ƒôÑ Descarregar Tabela 1 (Mensal)",
        ...
        with tab2:
            ...
            st.info("Ôä╣´©Å A tabela cumulativa soma os valores progressivamente, respondendo: 'At├® este m├¬s, quanto j├í foi pago?'")
            ...
            st.download_button(
                label="­ƒôÑ Descarregar Tabela 2 (Cumulativo)",
...
# =========================================================
# P├üGINA 3: DASHBOARD VISUAL
# =========================================================
elif pagina == PAGINAS[2]:
    st.title("­ƒôê 3. Dashboard Visual")
    
    if st.session_state.relatorio_final is None:
        st.warning("ÔÜá´©Å Primeiro, v├í ├á sec├º├úo 'Gerar Relat├│rio' e processe a tabela.")
    else:
        ...
        st.write("*(Nota: Os gr├íficos respondem aos filtros aplicados na p├ígina anterior.)*")
        
        filtros_txt = st.session_state.get('filtros_aplicados_texto', [])
        if filtros_txt:
            st.info("­ƒÄ» **Filtros Ativos:** " + " | ".join(filtros_txt))
        else:
            st.info("­ƒÄ» **Filtros Ativos:** Nenhum (A mostrar todos os dados)")
        ...
        kpi1.metric("­ƒÆ░ Valor Distribu├¡do", formata_mt_kpi(total_valor))
        kpi2.metric("­ƒæÑ Benefici├írios ├Ünicos", f"{float(total_benef):,.0f}")
        kpi3.metric("­ƒÆ│ Ficheiros / Transa├º├Áes", f"{float(total_pags):,.0f}")
        ...
        for col in rel_viz.columns:
            if col.startswith("Sexo_") or col == "Sem_G├¬nero":
                total_outros += pd.to_numeric(rel_viz[col], errors='coerce').sum()
                
        kpi_m.metric("­ƒæ¿ Homens (M)", f"{float(total_m):,.0f}")
        kpi_f.metric("­ƒæ® Mulheres (F)", f"{float(total_f):,.0f}")
        kpi_o.metric("­ƒæñ Genero N├úo Informados", f"{float(total_outros):,.0f}")
        
        st.divider()
        
        c1, c2 = st.columns(2)
        with c1:
            if 'F' in rel_viz.columns or 'M' in rel_viz.columns or 'Sem_G├¬nero' in rel_viz.columns:
                st.write("**Propor├º├úo de Sexo**")
                labels_sexo = []
                valores_sexo = []
                ...
                if 'Sem_G├¬nero' in rel_viz.columns:
                    labels_sexo.append('Sem G├¬nero')
                    valores_sexo.append(pd.to_numeric(rel_viz['Sem_G├¬nero'], errors='coerce').sum())
                    
                for col in rel_viz.columns:
                    if col.startswith("Sexo_") and col not in ['F', 'M', 'Sem_G├¬nero']:
        ...
        with c2:
            ...
            if freq_cols:
                st.write("**Frequ├¬ncia de Pagamentos**")
        ...
        if st.session_state.col_agrupamento:
            ...
            if 'Sem_G├¬nero' in rel_viz.columns: colunas_genero.append('Sem_G├¬nero')
            ...
            if len(colunas_genero) > 0:
                st.write(f"**Distribui├º├úo por: {eixo_x} (Desagregado por Sexo)**")
                ...
                st.write(f"**Benefici├írios ├Ünicos por: {eixo_x}**")
...
# =========================================================
# BOT├òES DE NAVEGA├ç├âO DE P├üGINA (Fixos em baixo)
# =========================================================
st.markdown("""
    <style>
    /* Espa├ºo para o rodap├® n├úo sobrepor a tabela ou gr├íficos */
    .block-container {
        padding-bottom: 100px !important;
    }
    
    /* Fixar a ├║ltima linha de colunas (nossos bot├Áes) ao fundo da janela */
    ...
    /* Ajustar largura caso a sidebar lateral esteja vis├¡vel */
...
with c1:
    if st.session_state.pagina_atual != PAGINAS[0]:
        st.button("Ô¼à´©Å Voltar", on_click=ir_anterior, use_container_width=True)
...
with c3:
    if st.session_state.pagina_atual != PAGINAS[-1]:
        ...
        st.button("Avan├ºar Ô×í´©Å", on_click=ir_proximo, use_container_width=True, type="primary", disabled=not pode_avancar)
